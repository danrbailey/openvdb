...
struct MaxValueOp {
    ...
    {
        ValueT max = std::numeric_limits<ValueT>::lowest();
...
bool MaxMergeOp<TreeT>::operator()(NodeT& node, size_t) const
{
    ...
    for (auto iter = node.beginChildAll(); iter; ++iter) {
        ...
        for (const auto& mergeNode : mergeNodes) {
            ...
            if (mergeNode.isTile() || mergeNode.node->isChildMaskOff(pos)) {
                ...
                if (mergeValue > max) {
                    const bool mergeActive = mergeNode.isTile() ?
                        mergeNode.active : mergeNode.node->isValueMaskOn(pos);
                    max = mergeValue;
                    iter.setValue(max);
                    iter.setValueOn(mergeActive);
                ...
                const auto* mergeChild = mergeNode.node->getTable()[pos].getChild();
                merge_internal::MaxValueOp<TreeT> maxValueOp;
    ...
    for (const auto& it : mergeNodesToSteal) {
        ...
        std::unique_ptr<ChildT> mergeChild = mergeTree->template stealOrDeepCopyNode<ChildT>(ijk,
            std::numeric_limits<ValueT>::lowest());
        if (mergeChild) {
            // ensure there are no child values lower than the tile value it replaces
            ValueT mergeValue;
            const bool active = node.probeValue(ijk, mergeValue);
            merge_internal::ApplyTileMaxToNodeOp<TreeT> applyOp(mergeValue, active);
            applyOp.run(*mergeChild);
            node.addChild(mergeChild.release());
        }
    }
        ...
        if (mergeLeaf) {
            ...
            for (Index i = 0; i < LeafT::SIZE; i++) {
                if (mergeValue > data[i]) {
                    data[i] = mergeValue;
                    leaf.setActiveState(i, mergeActive);
                }
            }
        }
    }
