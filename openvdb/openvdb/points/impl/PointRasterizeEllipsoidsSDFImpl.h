...
#define OPENVDB_ELLIPSOID_PROJECT_ONTO_SURFACE 1
    ...
    public rasterize_sdf_internal::SphericalTransfer<SdfT, PositionCodecT, RadiusType, CPG>
{
    ...
    inline bool startPointLeaf(const PointDataTree::LeafNodeType& leaf)
    {
        ...
        if (mIndices.hasWorldSpacePosition()) {
            mPositionWSHandle.reset(new PwsHandleT(leaf.constAttributeArray(mIndices.positionws)));
        }
        return ret;
    }
                    ...
                    const CoordBBox& bounds)
    {
        ...
        const math::Mat3s ellipsoidTransform = mRotationHandle->get(id);
        ...
        //  apart
        const Vec3d max = calcEllipsoidBoundMax(ellipsoidTransform.timesDiagonal(radius)) + r.halfband();
        ...
        const Vec3d radInv2 = 1.0f / (radius * radius);
        const math::Mat3s ellipsoidInverse = ellipsoidTransform.transpose();

        // Instead of trying to compute the distance from a point to a rotated ellipse,
        // stamp the ellipsoid by deforming the distance to the iterated voxel
        // by the inverse rotation. Then calculate the distance to the axis-aligned ellipse.
        ...
        for (Coord c = a; c.x() <= b.x(); ++c.x()) {
            ...
            for (c.y() = a.y(); c.y() <= b.y(); ++c.y()) {
                ...
                for (c.z() = a.z(); c.z() <= b.z(); ++c.z()) {
                    ...
                    const Vec3d xyzradial = (xyradial + Vec3d(z*inv[2], z*inv[5], z*inv[8]));

                    RealT k1 = (xyzradial * radInv).length();
                    RealT k2 = (xyzradial * radInv2).length();

                    ValueT d = this->mDx * (k1 * (k1 - 1.0) / k2); // calc distance and then scale by voxelsize to convert to ws
                    ...
                    const typename SettingsT::FilterType& filter)
{
    ...
    if (settings.radius.empty())
    {
        ...
        Vec3i width;
        {
            ...
            EllipsSurfaceMaskOp<FixedBandRadius<Vec3f>, MaskTreeT, InterrupterType>
    ...
    else
    {
        ...
        Vec3i width;
        {
            ...
            // pass radius scale as index space
            EllipsSurfaceMaskOp<VaryingBandRadius<RadiusT, Vec3f>, MaskTreeT, InterrupterType>
                op(points.transform(), *transform, rad, halfband, indices);
            tbb::parallel_reduce(manager.leafRange(), op);
